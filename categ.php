...
<div class='id'>
	<?php	
	$resultado = $_GET['categoria_id'];
	$sqlQuery = "select * from categoria where id=".$resultado."";
	$mysqlQuery = mysqli_query($conexao, $sqlQuery);
		while($search=mysqli_fetch_assoc($mysqlQuery)) { 
			echo "<h1 style='color: #68c2ff;'>".$search['categoria']."</H1>";
		}
	?>	
	...
	$resultado = $_GET['categoria_id'];
  $sqlQuery = "select * from vara where categoria_id=".$resultado."";
	$mysqlQuery = mysqli_query($conexao, $sqlQuery);
